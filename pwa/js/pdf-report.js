...
export async function renderInterventionPDF(report) {
  ...
  const isFolhaAvariasPdf = report.serviceType === 'folha_intervencao_avarias';
  let y = isFolhaAvariasPdf
    ? drawFolhaMirrorHeader(doc, clientMeta, job?.numeroOrdem ?? null, techName)
    : drawTopRowWithClientBlock(doc, clientMeta, job?.numeroOrdem ?? null, techName);
  ...
  const closingOpts = {
    ...
    skipClosingDiagnostic: isFolhaAvariasPdf,
    folhaLayout: isFolhaAvariasPdf,
  };
...
function formatFolhaOrdemDisplay(numeroOrdem) {
  if (numeroOrdem == null || numeroOrdem === '') return 'Ordem Nº: —';
  const padded = String(numeroOrdem).padStart(2, '0');
  return `Ordem Nº: OP-2026-${padded}`;
}
...
/** Cabeçalho bilateral — esquerda: logo + ordem/técnico; direita: cliente (nome + morada) */
function drawTopRowWithClientBlock(doc, clientMeta, numeroOrdem = null, techName = '—') {
  ...
  doc.setTextColor(...TEXT_DARK);
  const ordemTechLines = pdfSplitText(
    doc,
    formatOrdemTechnicianLine(numeroOrdem, techName),
    leftColW,
  );
  doc.text(ordemTechLines, MARGIN, leftY);
  leftY += ordemTechLines.length * 4.2 + 2;
  ...
  let blockContentH = nameLines.length * 4.2 + 2;
...
/** Cabeçalho espelho — coluna esq.: logótipo + ordem/técnico; col. dir.: cliente (topo, alinhado à direita) */
function drawFolhaMirrorHeader(doc, clientMeta, numeroOrdem = null, techName = '—') {
  ...
  doc.setFontSize(PDF_FONT_CAPTION);
  doc.setTextColor(...TEXT_MUTED);
  const addrAll = [clientMeta.addressLine, clientMeta.addressSubline].filter(Boolean);
  addrAll.forEach((addrPart) => {
  ...
  pdfSplitText(doc, formatFolhaOrdemDisplay(numeroOrdem), leftColW).forEach((line) => {
...
async function drawFolhaAvariasBody(doc, y, values, service, pdfContext) {
  y = await drawFolhaMachineGrid(doc, y, values, pdfContext);
  y = await drawFolhaWorkBlock(doc, y, values, service);
  y = await drawFolhaClosingDataBlock(doc, y, values);
  return y;
}

function drawFolhaMachineGridCell(doc, text, x, y, w, h) {
  pdfSetFont(doc, 'normal');
  doc.setFontSize(PDF_FONT_BODY);
  doc.setTextColor(...TEXT_DARK);
  const lines = pdfSplitText(doc, text, w - 8);
  const lineH = 4.2;
  const textY = y + Math.max(4, (h - lines.length * lineH) / 2 + 3);
  doc.text(lines, x + 4, textY);
}

async function drawFolhaMachineGrid(doc, y, values, pdfContext) {
  const resolveMachine = (spec) => {
    let fallback = null;
    if (spec.id === 'numero_de_serie') {
      fallback = pdfContext?.forkliftSerial || pdfContext?.report?.forkliftSerial || null;
    }
    return resolvePdfStandardFieldValue(values, spec, fallback);
  };

  const marca = resolveMachine(PDF_STANDARD_MACHINE_SPECS[0]);
  const modelo = resolveMachine(PDF_STANDARD_MACHINE_SPECS[1]);
  const serie = resolveMachine(PDF_STANDARD_MACHINE_SPECS[2]);
  ...
  const rowH = 10;
  const gridH = rowH * 2;
  const blockH = titleH + gridH + 8;
  y = ensureKeepTogetherBlock(doc, y, blockH);

  pdfSetFont(doc, 'bold');
  doc.setFontSize(PDF_FONT_CAPTION);
  doc.setTextColor(...CORPORATE_BLUE);
  doc.text('INFORMAÇÕES DA MÁQUINA', MARGIN, y + 4);
  ...
  doc.rect(MARGIN, gridY, CONTENT_W, gridH, 'FD');
  doc.line(MARGIN, gridY + rowH, MARGIN + CONTENT_W, gridY + rowH);
  doc.line(MARGIN + halfW, gridY, MARGIN + halfW, gridY + rowH);
  doc.line(MARGIN + thirdW, gridY + rowH, MARGIN + thirdW, gridY + gridH);
  doc.line(MARGIN + thirdW * 2, gridY + rowH, MARGIN + thirdW * 2, gridY + gridH);

  drawFolhaMachineGridCell(doc, folhaGridCell('Marca', marca), MARGIN, gridY, halfW, rowH);
  drawFolhaMachineGridCell(doc, folhaGridCell('Modelo', modelo), MARGIN + halfW, gridY, halfW, rowH);
  drawFolhaMachineGridCell(doc, folhaGridCell('Nº Série', serie), MARGIN, gridY + rowH, thirdW, rowH);
  drawFolhaMachineGridCell(
    doc,
...
async function drawFolhaWorkBlock(doc, y, values, service) {
  for (const spec of PDF_STANDARD_WORK_SPECS) {
    const text = String(resolvePdfStandardFieldValue(values, spec) || '').trim();
    if (!text) continue;
    y = drawFolhaBoundedTextField(doc, y, spec.label, text);
  }

  const materialField = resolveServiceMaterialField(service);
  if (!materialField) return y;
  ...
  return drawFolhaMaterialTable(doc, y, rows);
}

async function drawFolhaMaterialTable(doc, y, rows) {
  ...
  doc.setFontSize(PDF_FONT_CAPTION);
  doc.setTextColor(...CORPORATE_BLUE);
  doc.text(getMaterialTablePdfLabel().toUpperCase(), MARGIN, y + 4);
  y += 8;
...
function resolveFolhaEstadoMaquinaDisplay(values = {}) {
  const estado = String(values.estado_maquina || '').trim();
  if (estado) return estado;
  if (resolvePedidoOrcamentoValue(values)) return 'Pedido de Orçamento';
  return '—';
}
...
function drawPdfCheckbox(doc, x, y, checked) {
  ...
  if (checked) {
    ...
    pdfSetFont(doc, 'bold');
    doc.setFontSize(7);
    doc.text(pdfStatusGlyph('ok'), x + 0.35, y + 3.1);
  }
}
...
/** Bloco superior — data do serviço (esq.) + nº de visitas (dir.) */
function drawServiceInfoBlock(doc, y, meta) {
  ...
  if (meta.numeroVisitas != null) {
    ...
    doc.setTextColor(...TEXT_DARK);
    doc.text(
      pdfSafeText(`Nº de Visitas: ${pdfDisplayValue(meta.numeroVisitas)}`),
      rightX,
      rightY,
      { align: 'right' },
    );
...
async function drawPedidoOrcamentoPdfBlock(doc, y, values, options = {}) {
  ...
  if (!options.alwaysShow && !hasPedidoField) return y;
  ...
  const simX = MARGIN + 48;
  const naoX = simX + 22;
  drawPdfCheckbox(doc, simX, y - 3.5, pedido);
  pdfSetFont(doc, pedido ? 'bold' : 'normal');
  doc.setFontSize(PDF_FONT_CAPTION);
  doc.setTextColor(...TEXT_DARK);
  doc.text('Sim', simX + 5, y);

  drawPdfCheckbox(doc, naoX, y - 3.5, !pedido);
  pdfSetFont(doc, !pedido ? 'bold' : 'normal');
  doc.text('Não', naoX + 5, y);
  y += 8;
...
function planReportClosingProfile(doc, y, opts) {
  if (opts.folhaLayout) return FOLHA_CLOSING_PROFILE;
...
function drawPageFooter(doc, _reportId, options = {}) {
  const total = doc.getNumberOfPages();
  for (let i = 1; i <= total; i++) {
    if (options.lastPageOnly && i < total) {
      doc.setPage(i);
      pdfSetFont(doc, 'normal');
      doc.setFontSize(PDF_FONT_CAPTION);
      doc.setTextColor(...TEXT_MUTED);
