...
function patchAuthRolesJs(config, filePath) {
  const source = fs.readFileSync(filePath, 'utf8');
  const startIdx = source.indexOf(RH_CONFIG_START);
  const endIdx = source.indexOf(RH_CONFIG_END);
  if (startIdx < 0 || endIdx < 0 || endIdx <= startIdx) {
    throw new Error(`Marcadores RH_CONFIG não encontrados em ${filePath}`);
  }
...
patchAuthRolesJs(config, authRolesPath);
patchAuthRolesJs(config, apiAuthRolesPath);
console.log(`[sync:rh-config] ${jsonPath} + API + auth-roles-core.js (${config.emails.length} e-mails RH)`);
