...
describe('rh-admin-config', () => {
  it('JSON da API e auth-roles-core.js estão alinhados com UTILIZADORES', () => {
    ...
    assert.ok(fs.existsSync(apiAuthRolesPath), 'Correr npm run sync:rh-config (server-lib/auth-roles.js)');
    ...
    assert.ok(expected.emails.every((email) => apiSrc.includes(email)), 'server-lib/auth-roles.js desatualizado');
